...
export default function MaxGraph() {
  ...
  // Типы узлов держим в состоянии: раньше они пересчитывались из sim.current
  // прямо в теле рендера, а читать ref во время рендера React 19 запрещает.
  ...
  async function connect() {
    ...
    try {
      ...
      s.running = true;
      setNodeTypes(Array.from(new Set(s.nodes.map((n) => n.type))));
      setStats(g.stats);
      setStatus('ok');
    } catch (e: unknown) {
      setError(
  ...
  // Раньше типы читались из sim.current прямо в теле рендера — React 19 это
  // запрещает. Держим их в состоянии и обновляем там же, где меняются узлы.
