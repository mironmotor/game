...
// Потолка у пула нет: массивы растут по мере надобности. Сколько частиц живёт —
// решает физика (рождение × время жизни e^(−t/τ)), а не жёсткий лимит.
const INITIAL_CAP = 8192;
const TWO_PI = Math.PI * 2;

// Единственная ручка плотности. Потолка на число частиц нет вообще: пул растёт
// сам, а сколько их живёт — определяет равновесие «рождение × время жизни».
// DENSITY просто задаёт темп рождения. 1 — плотно и плавно; 6 — огненный шланг
// на сотни тысяч частиц (на слабом железе превратится в слайд-шоу).
const DENSITY = 1;
...
export default function MaxVision() {
  ...
  const sim = useRef({
    px: new Float32Array(INITIAL_CAP),
    py: new Float32Array(INITIAL_CAP),
    vx: new Float32Array(INITIAL_CAP),
    vy: new Float32Array(INITIAL_CAP),
    born: new Float32Array(INITIAL_CAP),
    life: new Float32Array(INITIAL_CAP),
    hue: new Float32Array(INITIAL_CAP),
    sat: new Float32Array(INITIAL_CAP),
    count: 0,
    cap: INITIAL_CAP,
    t: 0,
    rot: 0,
  ...
  useEffect(() => {
    ...
    // Слой частиц: рисуем их в собственный пиксельный буфер (аддитивно,
    // с насыщением) и переносим на холст ОДНИМ вызовом. Стоимость одной
    // частицы — несколько арифметических операций, поэтому их число может
    ...
    // Расширить пул вдвое — когда живых стало больше ёмкости.
    const grow = () => {
      const s = sim.current;
      const cap = s.cap * 2;
    ...
    // Вычеркнуть погасшую: на её место переносим последнюю живую.
    ...
    const emit = (now: number, x: number, y: number, vx: number, vy: number, hue: number, sat: number, life: number) => {
      const s = sim.current;
    ...
    const loop = (ts: number) => {
      ...
      for (let b = 0; b < VISION_BANDS; b++) {
        ...
        const eb = spec[b];
        // Сколько частиц выпустить из этой полосы (∝ её энергии). Потолка нет —
        // равновесие задаёт только темп рождения × время жизни. Хочешь гуще —
        // поднимай DENSITY, счёт ничем не ограничен (но кадры дешевеют не бесплатно).
        const count = eb > 0.02 ? Math.round((0.5 + eb * 7) * DENSITY * (dt * 60)) : 0;
        for (let k = 0; k < count; k++) {
          ...
          emit(now, cx + ux * r0, cy + uy * r0, ux * speed, uy * speed, hue, sat, life);
        }
      }

      // палец рисует светом
      if (s.pointer.active) {
        const brush = Math.round(26 * (dt * 60));
        for (let k = 0; k < brush; k++) {
          const ang = Math.random() * TWO_PI;
          const sp = 30 + Math.random() * 120;
          emit(now, s.pointer.x, s.pointer.y, Math.cos(ang) * sp, Math.sin(ang) * sp, 210 + Math.random() * 120, 0.25, 1.0 + Math.random());
        }
      }
      ...
      if (activeIdle && Math.random() < 0.5) {
        const ang = Math.random() * TWO_PI;
        emit(now, cx, cy, Math.cos(ang) * 40, Math.sin(ang) * 40, 220 + Math.random() * 80, 0.5, 1.4);
      }
      ...
      // частицы — считаем движение и сразу впечатываем в пиксельный буфер
      ...
      let i = 0;
      while (i < s.count) {
        const decay = expE(-(now - s.born[i]) / s.life[i]); // e^(−t/τ)
        if (decay < 0.05) {
        ...
        const px = s.px[i];
        const py = s.py[i];
        // улетевшую за кадр гасим — она больше не вернётся
        ...
        if (data) {
          ...
          const white = 1 - s.sat[i]; // громче — белее
          const boost = clamp(0.5 + sp / 160000 + white * 0.4, 0, 1);
          const a = decay * (0.22 + white * 0.3) * (0.6 + boost * 0.7);
          const hi = hueIndex(s.hue[i]);
          const r = (HUE_LUT[hi] + (255 - HUE_LUT[hi]) * white) * a;
          const g = (HUE_LUT[hi + 1] + (255 - HUE_LUT[hi + 1]) * white) * a;
          const bl = (HUE_LUT[hi + 2] + (255 - HUE_LUT[hi + 2]) * white) * a;
          const xi = px | 0;
          const yi = py | 0;
          if (xi >= 0 && xi < w && yi >= 0 && yi < h) {
            ...
            data[o + 2] += bl;
            data[o + 3] = 255;
            // мягкое ядро у ярких частиц — крест из соседей вполсилы
            if (decay > 0.45) {
              const hr = r * 0.5;
              const hg = g * 0.5;
              const hb = bl * 0.5;
              if (xi + 1 < w) { const p = o + 4; data[p] += hr; data[p + 1] += hg; data[p + 2] += hb; data[p + 3] = 255; }
              if (xi - 1 >= 0) { const p = o - 4; data[p] += hr; data[p + 1] += hg; data[p + 2] += hb; data[p + 3] = 255; }
              if (yi + 1 < h) { const p = o + w * 4; data[p] += hr; data[p + 1] += hg; data[p + 2] += hb; data[p + 3] = 255; }
              if (yi - 1 >= 0) { const p = o - w * 4; data[p] += hr; data[p + 1] += hg; data[p + 2] += hb; data[p + 3] = 255; }
            }
          }
        }
        i++;
      ...
      // весь слой частиц — одним переносом, аддитивно поверх дыма
      if (lctx && img) {
      ...
      if (hudAcc >= 0.2) {
        hudAcc = 0;
        setHud({ bass, mid, treble, f0: Math.round(sig.f0), alive: s.count });
      }
