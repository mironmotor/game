...
function GameContent() {
  ...
  const handleStart = async (mode: 'high' | 'normal' | 'low') => {
    ...
    try {
      ...
      const activeTasks = tasks.filter(t => t.status !== 'completed' && t.status !== 'failed');
      let initialMsg = "START_GAME_COMMAND";
      ...
      if (activeTasks.length > 0) {
        ...
        initialMsg = `SYSTEM_RESUME_COMMAND. Пользователь вернулся в систему. Текущий уровень энергии/настроя: ${mode}.
Ниже приведена текущая сводка задач:
${taskContext}

Проанализируй эти задачи. Напиши развернутое, атмосферное и погружающее приветствие в стиле Джарвиса.
1. Расскажи про текущую ситуацию и оставшиеся задачи.
2. Дай рекомендации, за что лучше взяться сейчас, учитывая текущий уровень энергии.
3. В самом конце обязательно спроси, с чего мы начнем или какова наша главная задача на сегодня (MGR-3).`;
      } else {
        initialMsg = `START_GAME_COMMAND. Пользователь вернулся в систему. Текущий уровень энергии/настроя: ${mode}.
Активных задач в системе сейчас нет — чистое поле.

Проанализируй это состояние. Напиши развернутое, атмосферное и погружающее приветствие в стиле Джарвиса.
1. Поприветствуй и обозначь, что система полностью чиста.
2. Дай стратегические рекомендации о том, как лучше всего заполнить день и на чем сфокусироваться.
3. В самом конце обязательно спроси: "Какова наша главная задача на сегодня (MGR-3)?".`;
      }
