...
# --- 7. Запуск --------------------------------------------------------------
# --- 7. Остановка старого экземпляра ----------------------------------------
# Telegram отдаёт обновления только одному процессу: если бот уже запущен в
# другом окне, оба будут перехватывать запросы друг у друга и сыпать
# TelegramConflictError. Поэтому сначала гасим прежний.
if pgrep -f 'python -m bot.main' >/dev/null 2>&1; then
  pkill -f 'python -m bot.main' 2>/dev/null || true
  sleep 2
  warn "Остановил ранее запущенный экземпляр бота"
fi
