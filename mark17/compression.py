...
# Порядок = приоритет: _stem отсекает ПЕРВОЕ совпавшее окончание, поэтому
# длинные обязаны стоять раньше коротких. Перепутать здесь порядок — значит
# тихо получить «написа» вместо «напис» и разошедшийся разговор.
#
# Прошедшее время появилось здесь после замера на стенде: «написал» не
# отсекалось вовсе и не совпадало с «написать», хотя человек рассказывает о
# сделанном ровно так — прошедшим временем. Из пятнадцати пар живых
# перефразировок ядро не узнавало шесть, и эта была самой обидной.
_ENDINGS = (
    "ениями", "ениям", "ениях", "ением", "ования", "ование", "ований",
    "ывать", "ивать", "евать", "овать", "имать", "инать",
    "ались", "ялись", "ились", "улись", "ылись",
    "ался", "ился", "ялся", "улся",
    "нуть", "ять", "ать", "еть", "ить", "уть", "ыть",
    "али", "яли", "ели", "или", "ули", "ыли",
    "ала", "яла", "ела", "ила", "ула", "ыла",
    "ало", "яло", "ело", "ило", "уло", "ыло",
    "ами", "ями", "ого", "его", "ому", "ему", "ыми", "ими", "ей", "ой",
    "ах", "ях", "ов", "ев", "ый", "ий", "ая", "яя", "ое", "ее", "ые", "ие",
    "ом", "ем", "ам", "ям", "ть", "ся",
    "ал", "ял", "ел", "ул", "ол", "ла", "ли", "ло", "ил", "ыл",
    "а", "я", "о", "е", "ы", "и", "й", "ь", "у", "ю",
)
_STOP = {
    ...
    "про", "насчет", "насчёт", "он", "она", "они", "оно",
    "потихоньку", "пока", "снова", "опять", "давай", "надо",
...
# Сколько букв в начале двух основ должно совпасть, чтобы считать их одним
# словом. Нужно из-за вида глагола: «поднять» даёт «подн», «поднимаю» даёт
# «поднима», и никакой список окончаний их не сведёт — это разные основы, а
# не разные хвосты. Человек же меняет вид свободно и об одном и том же.
#
# Четыре, а не три: на трёх «проект» и «продать» слиплись бы по «про». На
# четырёх этого уже не происходит, а «подн»/«поднима» и «запус»/«запуст»
# сходятся. Порог проверен на стенде — по обеим метрикам сразу, потому что
# склеивать лишнее так же вредно, как не склеивать нужное.
STEM_PREFIX = 4
...
def _same_stem(x: str, y: str) -> bool:
    ...
    if len(shorter) < STEM_PREFIX:
        return False
    common = 0
    for cx, cy in zip(shorter, longer):
        if cx != cy:
            break
        common += 1
...
def similarity(a: str | set[str], b: str | set[str]) -> float:
    ...
    Совпадением считается не только точное равенство основ, но и общее
    начало длиной STEM_PREFIX — иначе вид глагола рвёт одну тему надвое.

    Каждая основа участвует в паре один раз. Без этого «подн» из одного
    текста засчиталось бы и к «поднима», и к «поднял» из другого, и
    похожесть перевалила бы за единицу — метрика начала бы врать в свою
    пользу ровно там, где на неё и полагаются.
    """
    ...
    for x in sa:
        for i, y in enumerate(free):
            if _same_stem(x, y):
                ...
                free.pop(i)
                break
    return 2 * matched / (len(sa) + len(sb))
