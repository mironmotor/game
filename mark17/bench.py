...
def _is_substantive(question: str, answer: str) -> bool:
    """Ответ по делу — тот, в котором есть что сделать.

    Первая версия мерила длину и не-эхо, и на этом обманулась: приборная
    панель («Я понял запрос… уверенность 21%… закрепить как паттерн»)
    длинная и на вопрос не похожа, поэтому засчитывалась как полноценный
    ответ. Покрытие показывало 12 из 12 там, где на самом деле было 11:
    один вопрос плана не получал вовсе.

    Поэтому признак теперь не размер, а наличие разбора по шагам —
    единственное, чем предметный ответ этого ядра отличается от вежливой
    отписки. «Про это у меня ничего нет» — честный ответ, но не предметный,
    и считать его покрытием значит врать себе.
    """
